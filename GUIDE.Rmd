...
`sample_summary: counts.min` setting) no genotyping will be attempted.  Next
only those sequences accounting for at least a minimum fraction of the
remaining reads are considered.  (The default value is
`r cfg("min_allele_abundance")`.  This can be changed via the
`min_allele_abundance` setting.)  Sequences that are marked as potential stutter
or other artifacts (via the Stutter and Artifact columns of the table) or
contain ambiguous sequence content (via the Ambiguous column) are excluded
next.
